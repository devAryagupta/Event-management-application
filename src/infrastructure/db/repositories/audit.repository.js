...
class AuditRepository extends AuditRepositoryPort {
    ...
    async create(audit) {
        const result = await this.baseRepository.query(
            `INSERT INTO audit_logs (event_id, changed_by, change_type, previous_value, new_value, actor_timezone)
             VALUES ($1, $2, $3, $4, $5, $6)
             RETURNING *`,
            [
                audit.eventId,
    ...
    async findByChangedBy(changedBy) {
        const result = await this.baseRepository.query(
            'SELECT * FROM audit_logs WHERE changed_by = $1',
            [changedBy]
        );
        return this.baseRepository.mapmany(result, this.toEntity);
    }
